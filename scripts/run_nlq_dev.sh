...
set -euo pipefail
cd "$(dirname "$0")/.." || exit 1
# NLQ follows DCL's current entity (config.get_tenant_id) — it is no longer pinned
# by AOS_TENANT_ID, so there is nothing to capture/restore around the env sourcing.
set -a
